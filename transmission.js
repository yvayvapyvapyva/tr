...
export class Transmission{
  ...
  step(dt){
    ...
    } else if(PHYS.PEDAL_RETURN_BRAKE && this.brakeReturn && !this.brakeDragging){
    ...
    if(PHYS.PEDAL_RETURN_GAS && this.gasReturn){
    ...
    if(PHYS.PEDAL_RETURN_CLUTCH && this.clutchReturn){
    ...
    /* привод «связан»: передача включена и сцепление введено — только тогда
       колёса могут толкать двигатель. При выжатом сцеплении (или на нейтрали)
       привода от колёс к коленвалу нет. */
    const drivelineOn = ratio!==0 && e>0.02;
    ...
    /* ------------------- ДВИГАТЕЛЬ + СЦЕПЛЕНИЕ -------------------
       Модель «замок/проскальзывание» с событийным захватом: если
       сцепление способно удержать момент, коленвал и диск крутятся
       как одно целое, иначе — проскальзывание с моментом = ёмкость
       сцепления. Замыкание скольжения ловим аналитически внутри кадра. */
    ...
    if(this.engineState==='running'){
      ...
      /* закрытый дроссель: насосные потери (торможение двигателем)
         ...
         полностью закрытого (максимум). Действуют ТОЛЬКО когда привод
         связан (передача + сцепление введено): колёса толкают двигатель,
         и торможение закрытым дросселем реально ощущается. При выжатом
         сцеплении (или на нейтрали) двигатель свободен — насосные потери
         не тормозят его, как в реальной машине. На холостых их держит
         регулятор (gov>0), поэтому на работающем моторе они не мешают. */
      const closure=Math.max(0, Math.min(1, 1-this.gasP/PHYS.GAS_CLOSE_POS));
      if(drivelineOn && gov<=0.01 && closure>0)
      ...
      Td=engDragNm(rpm)+(drivelineOn?pumpDragNm(rpm):0);
    } else {
      /* двигатель не работает: горения нет, только трение и компрессия.
         Колёса через сцепление при этом могут раскрутить коленвал —
         насосные потери закрытого дросселя тормозят только когда привод
         связан (даёт торможение двигателем и работу «с толкача»), при
         выжатом сцеплении двигатель свободно раскручивается колёсами. */
      const rpm=we*RPM_C;
      Td=engDragNm(rpm)+(drivelineOn?pumpDragNm(rpm):0);
    }
    ...
    let spin=false, tracL=0;                 // пробуксовка и реальный момент дороги
    {
      ...
      const TeN=Te-Td;                       // чистый момент на коленвале
      const I2full=ratio?(PHYS.ID+IwEff/(ratio*ratio)):PHYS.ID; // диск+колёса+кузов
      ...
      /* Жёсткий замок против всей машины: считаем, чем сцепление реально
         нагрузит колонну в ЭТОМ кадре. Если скольжение за кадр не сойдётся
         к нулю (tFit>st) или ёмкость не держит установившийся замок
         (|fitT|>capE) — кадр буксует, и до колёс доходит вся ёмкость сцеп-
         ления; превысит она предел шин — колёса сорвёт в букс. Пример: сброс
         сцепления на полном газу, мотор на отсечке, диск стоит — скольжение
         ~400 рад/с закрывается за 0.3 с ≫ кадр, весь кадр колёса получают
         capE×передачу, на 82 л.с. это ~3170 Н·м против предела шин ~2960 Н·м.
         Замок же как установившийся момент почти нулевой — оценивать по нему
         нельзя, буксование бы пропало. SYNC_GAP не нужен: сходимость ловим
         аналитически, как в солвере couple ниже. */
      const aFit=(TeN-refS)/(PHYS.IE+I2full);
      const fitT=TeN-PHYS.IE*aFit;             // момент сцепления для жёсткого разгона с кузовом
      let sFit=we-wd;
      if(Math.abs(sFit)<1e-9) sFit=(fitT>=0?1:-1);
      const TcFit=sFit>0?capE:-capE;          // момент ёмкости в буксографии
      const dFit=(TeN-TcFit)/PHYS.IE-(TcFit-refS)/I2full; // рост скольжения
      const tFit=dFit*sFit<0?-sFit/dFit:Infinity;   // когда скольжение закроется в этом кадре
      const slipWholeFrame=!(Math.abs(fitT)<=capE) || tFit>=st;

      let tracNeed, TcT=0;
      if(!slipWholeFrame){
        /* скольжение закроется в кадре — запрос тяги от жёсткого разгона с кузовом */
        tracNeed=Mah*(aFit/ratio)+refS*ratio;
      } else {
        /* кадр буксует (или выжато): до колёс доходит только Tc */
      ...
      /* Событийное сцепление: коленвал (we, инерция IE, момент TeN) и колонна
         (wd, инерция I2, сопротивление Tload) соединены связью с предельным
         моментом capE. Моменты за кадр постоянны → ускорения и скольжение
         линейны. Скольжение, успевшее сойти к нулю внутри кадра, ловится
         точно в точке пересечения — без перелётов скоростей и перекачки
         энергии (из-за которых колонна раскачивалась на сотни рад/с, а
         выжатая машина «ехала вечно»). Если ёмкости не хватает даже на
         устойчивый замок (|Tlock|>capE), скольжение не замыкается: диски
         проходят друг сквозь друга со сменой знака в точке пересечения,
         без знакопеременной тряски кадр за кадром. */
      const couple=(I2,Tload)=>{
        const a=(TeN-Tload)/(PHYS.IE+I2);   // ускорение жёсткого замка
        const Tlock=TeN-PHYS.IE*a;          // момент сцепления, нужный для замка
        if(Math.abs(we-wd)<1e-9 && Math.abs(Tlock)<=capE){
          /* уже синхронны и сцепление держит — единый разгон весь кадр */
          locked=true; Tc=Tlock;
          we+=st*a; wd+=st*a;
          return;
        }
        let s=we-wd;
        if(s===0) s=(Tlock>=0?1:-1);        // разрыв замка — знак от перегруза
        const TcD=s>0?capE:-capE;           // ёмкость против скольжения
        const aE=(TeN-TcD)/PHYS.IE;
        const aD=(TcD-Tload)/I2;
        const ds=aE-aD;                     // рост скольжения
        const tC=ds*s<0?-s/ds:Infinity;     // время до смены знака скольжения
        if(st<tC){
          /* кадр целиком в текущем знаке скольжения */
          Tc=TcD;
          we+=aE*st; wd+=aD*st;
        } else if(Math.abs(Tlock)<=capE){
          /* скольжение сошлось к нулю — событийный захват, дальше замок */
          we+=aE*tC; wd+=aD*tC;
          locked=true; we=wd;
          const rem=st-tC;
          we+=a*rem; wd+=a*rem;
        } else {
          /* ёмкости на замок не хватает: скольжение меняет знак, диски
             проходят сквозь друг друга — момент сцепления разворачивается */
          we+=aE*tC; wd+=aD*tC;
          const rem=st-tC;
          we+=(TeN+TcD)/PHYS.IE*rem;
          wd+=(-TcD-Tload)/I2*rem;
        }
      };
      ...
      if(spin){
        ...
        const wSAcc=!slipWholeFrame?(TeN-tRef0)/(PHYS.IE+I2spin):0;
        const avW=!slipWholeFrame
          ?Math.abs((TeN-PHYS.IE*wSAcc)*ratio)
          :Math.abs(TcT*ratio);
        ...
        /* дорога держит — колонна с кузовом, обычное качение */
        couple(I2full,refS);
      } else {
        /* нейтраль: на второй стороне только диск и его сопротивление */
        couple(PHYS.ID, PHYS.DISC_DRAG*wd);
      }
    }
    ...
    /* отсечка должна резать ВЕСЬ жёсткий зацеп, а не только коленвал: если кадр
       щёлкнул зацепом (locked) и пара превысила отсечку сразу после реза коленвала
       то же самое прижимаем к диску — иначе диск остаётся чуть выше маховика,
       следующий кадр зацеп «расходится» на микропроскальзывание и разница держится.
       Свободный диск (сцепление выжато / скользит) не трогаем — он легально может
       крутиться выше отсечки от колёс на включённой передаче. */
    if(locked && wd>OVERREV_CUT) wd=OVERREV_CUT;
    /* красная зона: пока сцепление введено (e>0), диск и колёса не могут
       ...
       При ВЫЖАТОМ сцеплении (e≈0) диск от двигателя отсоединён, но жёстко
       связан с колёсами передачей — он легально раскручивается ими выше
       лимита (это ведомый диск КПП, а не коленвал). Резять его нельзя:
       иначе при переключении на большой скорости на низкую передачу машина
       мгновенно тормозится до скорости, отвечающей лимиту на новой передаче. */
    if(e>0.02){
